...
def _write_report(jobs: list, snaps: dict, output_file: str):
    ...
    for job in jobs:
        ...
        if name not in summary:
            ...
            if params:
                key = _canonical_name(
                    params["R"], params["Mstar"], params["Mbh"], params["beta"]
                )
                snap = snaps.get(key, {})
            summary[name] = {
                "Jobs": 0,
    ...
    with open(output_file, "w", newline="") as f:
        ...
        for name, info in sorted(
            summary.items(), key=lambda x: x[1]["CoreHours"], reverse=True
        ):
            writer.writerow(
                {
                    "RunName": name,
        ...
        writer.writerow(
            {
                "RunName": "TOTAL",
                "Jobs": "",
                "TotalCoreHours": f"{total_core_hours:.1f}",
                "Running": "",
                "SnapNum": "",
                "TimeDays": "",
                "TimeInTfb": "",
        ...
        f.write(
            f"# SBU budget remaining: {SBU_BUDGET - total_core_hours:.0f}"
...
def main(
    ...
    logger.info(
        "Monitoring {}, output to {}. Log at {}", input_dir, output_file, log_file
    )
